...
<div class="inner">
    ...
    <div class="contact-inner">
        <h2>いますぐ詳細資料をお届けします</h2>
        <form action="<?= $this->webroot ?>A02<?php
        if (isset($from)) {
            echo '/?from=' . $from;
        }
        ?>" id="formA01" method="post">
            <div class="form-row">
                <div class="mail-text">資料送付先メールアドレスをこちらにご入力下さい</div>
                <input type="email" name="email" placeholder="例）info@bell-face.com">
            </div>
            <div class="form-row text-center">
                <input type="submit" value="次へ" class="btnSubmit green">
            </div>
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                var validator = $("#formA01").validate({
                    rules: {
                        email: {
                            required: true,
                            email: true
